...
Since version 0.3, **cranly** includes functions for extracting information from `cranly_network` objects. All extractor functions in `cranly` try to figure out what `y` is in the statements
...
```{r}
package_network %>% maintained_by("Helen", flat = FALSE)
```
...
```{r}
word_cloud(package_network, maintainer = "Ioannis Kosmidis", exact = TRUE)
word_cloud(package_network, maintainer = "Achim Zeileis", exact = TRUE)
word_cloud(package_network, maintainer = "Edzer Pebesma", exact = TRUE)
```
