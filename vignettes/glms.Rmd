...
```
Note here that the package sub-network contains not only `glm_packages` but also all packages that are linked somehow with `glm_packages`.

In order to focusing on the sub-network with *only* `glm_packages`, we do
```{r}
