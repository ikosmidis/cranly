...
test_that("package_with with exact = FALSE returns correct results", {
    ...
    expect_identical(length(package_with(package_network, Inf, exact = FALSE, flat = TRUE)), 15031L)
    expect_identical(dim(package_with(package_network, Inf, exact = FALSE, flat = FALSE)), c(15031L, 65L))
    expect_identical(dim(package_with(author_network, Inf, exact = FALSE, flat = FALSE)), c(20439L, 2L))
})
...
test_that("author_with returns correct resuts", {
    ...
    expect_true(all(
        ...
        c("Alex Ioannides", "Ioannis N Athanasiadis", "Ioannis Tsamardinos",
          "Ioanna Manolopoulou", "Lazaros Ioannidis", "Ioannis Kosmidis",
          "Eleni Ioanna Delatola", "Ioana-Elena Oana")))
    expect_identical(length(author_with(package_network, Inf)), 20439L)
    expect_identical(length(author_with(author_network, Inf)), 20439L)
    expect_identical(dim(author_with(author_network, Inf, flat = FALSE)), c(20439L, 2L))
    expect_identical(dim(author_with(package_network, Inf, flat = FALSE)), c(15031L, 65L))
})

test_that("suggets, imports, linking_to works", {
    expect_true(all(suggested_by(package_network, "cranly", exact = TRUE) == c("knitr", "rmarkdown")))
    expect_equal(
        imported_by(package_network, "cranly", exact = TRUE),
        c("visNetwork", "colorspace", "igraph", "magrittr", "stringr",
          "ggplot2", "countrycode")
    )
    expect_equal(
        linked_by(package_network, "RcppArmadillo", exact = TRUE),
        "Rcpp"
    )
})
